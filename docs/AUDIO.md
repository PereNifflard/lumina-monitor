...
continu), mais le débit et la taille des trames avec du contenu restent à mesurer. **Demande au
fondateur : lancer une musique sur le téléphone pendant les 10 s de `audio-info 10 son.rtp`.**
...
Recommandation : **(c) tout de suite, (a) seulement si le fondateur veut le son *dans* l'app, en
...
2. **jusqu'au 9 septembre 2026 ce logiciel n'avait jamais ouvert de flux audio** — le chemin audio
   ...
   video`), ce qui reste à vérifier avec le fondateur.
...
### Dépannage (à recopier dans `docs/INSTALLATION.md`)
